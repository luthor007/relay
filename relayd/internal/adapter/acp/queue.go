...
// queue decouples the single reader goroutine from whoever consumes normalized
// events.
//
...
// straight from the reader would let a slow consumer stall the agent, including
// stalling our answer to a session/request_permission, which is the exact
// deadlock the contract forbids. So pushes never block and one pump goroutine
// does the blocking instead.
type queue struct {
	mu      sync.Mutex
	items   []event.Event
	closed  bool
...
// close marks the end of the stream. The pump drains what is already queued
// before closing the channel, so a consumer still sees the final TurnCompleted.
func (q *queue) close() {
	q.mu.Lock()
	if q.closed {
		q.mu.Unlock()
...
// once, and a leaked goroutine per dead session is worse than a truncated tail
// nobody was reading.
func (q *queue) abandon() { q.hardOnce.Do(func() { close(q.hard) }) }

// closeAndDrain closes the queue and gives the consumer up to grace to take
// what is left. It returns once the channel is closed.
...
// droppedCount is how many events a non-reading consumer never saw.
